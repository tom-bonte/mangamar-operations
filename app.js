    ...
    TIMES.forEach(time => {
        ...
        const appendSlot = (parentCol, mainTrip, conflictArray, boatId, timeSlot) => {
            ...
            if (totalTrips === 0) {
                ...
                empty.className = "w-full h-full rounded-2xl border border-dashed border-slate-300 hover:bg-white hover:shadow-sm cursor-pointer transition-all flex items-center justify-center group";
    ...
    let previewHtml = (trip.groups || []).map(group => {
        ...
        const guestsHtml = (group.guests || []).map(g => {
            ...
            return `<div class="flex justify-between items-center text-[10px] mb-1 last:mb-0 group/item">
                        <button onclick="event.stopPropagation(); openCustomerProfile('${g.dni}', '${g.nombre.replace(/'/g, "\\'")}')" 
                                class="truncate pr-2 font-bold text-white hover:text-blue-400 text-left transition-colors cursor-pointer flex-1">
                            ${g.nombre}
                        </button>
