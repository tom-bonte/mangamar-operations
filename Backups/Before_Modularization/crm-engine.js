...
function calculateDivePrice(historyItem) {
    ...
    if (historyItem.type === 'producto' || historyItem.type === 'servicio' || historyItem.type === 'pago') {
        ...
        // Try to find in dynamic prices first (Enhanced Case-Insensitive Matching)
        const dynamicMatch = (window.dynamicPrices || []).find(p => {
            if (!p.name) return false;
            const pNameLower = p.name.toLowerCase().trim();
            return pNameLower === siteLower || 
                   pNameLower === 'inmersión ' + siteLower || 
                   pNameLower.includes(siteLower) && siteLower.length > 3;
        });
        ...
        if (dynamicTasa !== null) {
            ...
            tasa = 10;
        } else if (['cala', 'shore', 'aula', 'naranjito'].includes(siteLower)) {
            tasa = 0;
        } else if (siteLower) {
        ...
        if (historyItem.computer === 'INC') {
            ...
            computer = (historyItem.computerPrice !== undefined && historyItem.computerPrice !== null) ? historyItem.computerPrice : 7;
        }
    }
...
window.openCustomerProfile = async function (dni, nombre, isNavBackForward = false, targetTab = 'caja') {
    ...
    try {
        ...
        docsArray.forEach(doc => {
            ...
            // Recalculate total from individual components
...
window.renderFichaFromCache = function(dni, targetTab = 'caja') {
    ...
    if (elDeuda) {
        ...
        document.getElementById('ficha-caja-senal').innerText = `- ${deposit.toFixed(2)} €`;
...
window.updateCustomerDiscount = async function (val) {
    ...
    showToast("⏳ Guardando descuento...", 1000);
    ...
    try {
        // 2. Persistent Save to Master Directory
        await db.collection("mangamar_directory").doc("master_list").update({ clients: customerDatabase });

        // 3. Fallback save to individual doc (redundancy)
        await db.collection('mangamar_customers').doc(window.activeFichaDni).set({ 
            discount: disc, 
            discountType: discType 
        }, { merge: true });

        showToast("✅ Descuento guardado correctamente");
    } catch (e) {
    ...
    window.openCustomerProfile(window.activeFichaDni, currName); 
}
...
window.renderCrmTable = function () {
    ...
    let filtered = customerDatabase.filter(c => {
        ...
        const searchTarget = window.normalizeSearchString(`${cName} ${c.dni || ''} ${c.email || ''} ${c.telefono || ''}`);
    ...
    try {
        ...
        data.clients.forEach(sheetClient => {
            ...
            if (existing) {
                ...
                if (!existing.insurance && sheetClient.insurance && sheetClient.insurance.type) { existing.insurance = sheetClient.insurance; modified = true; }
